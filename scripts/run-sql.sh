...
FILES=(01_schema.sql 05_automation_schema.sql 02_plsql.sql 06_automation_plsql.sql 03_seed.sql)
...
# ------------------------------------------------------------ compile status
log "verifying that ERP_OPERATIONS + ERP_AUTOMATION compiled VALID and all tables exist"
verify="$(mktemp)"
verify_log="$(mktemp)"
...
printf '%s\n' \
  ...
  " WHERE object_name IN ('ERP_OPERATIONS', 'ERP_AUTOMATION');" \
  "EXIT;" > "$verify"
...
if [ "$INV" != "0" ]; then
  ...
  FAILED=1
elif [ "$PKG1" -lt 2 ] || [ "$PKG2" -lt 2 ]; then
  err "packages incomplete (ERP_OPERATIONS valid objects: $PKG1, ERP_AUTOMATION: $PKG2 - need 2 + 2)"
  FAILED=1
else
  ok "packages ERP_OPERATIONS + ERP_AUTOMATION (spec + body) are VALID"
fi

if [ "$TBL" -ge 20 ]; then
  ok "schema contains $TBL tables (13 core + 7 automation), $PRC package subprograms exposed"
else
  err "only $TBL tables found - expected 20 (13 core + 7 automation)"; FAILED=1
fi
