...
BEGIN
  ...
  -- Rebuild the opening balances of every material used by the BOM of
  -- FG_RUNNER_PRO_42 so that the ledger invariant
  --   STOCK.QTY = SUM(INVENTORY_TRANSACTION.QTY)
  -- holds again. Any movement produced by earlier runs of this script or by
  -- the API smoke test is removed together with its ledger rows.
  FOR m IN (SELECT i.ID AS ITEM_ID, d.SEED_QTY
              FROM (SELECT 'MAT_RUBBER_01' AS CODE, 40  AS SEED_QTY FROM DUAL UNION ALL
                    SELECT 'MAT_MESH_01'   AS CODE, 500 AS SEED_QTY FROM DUAL UNION ALL
                    SELECT 'MAT_THREAD_01' AS CODE, 200 AS SEED_QTY FROM DUAL UNION ALL
                    SELECT 'MAT_GLUE_01'   AS CODE, 100 AS SEED_QTY FROM DUAL UNION ALL
                    SELECT 'MAT_BOX_01'    AS CODE, 600 AS SEED_QTY FROM DUAL) d
              JOIN ITEM i ON i.CODE = d.CODE)
  LOOP
    DELETE FROM INVENTORY_TRANSACTION WHERE ITEM_ID = m.ITEM_ID;
    UPDATE STOCK
       SET QTY = m.SEED_QTY, UPDATED_AT = SYSDATE
    ...
    IF SQL%ROWCOUNT = 0 THEN
      ...
      VALUES ((SELECT ID FROM WAREHOUSE WHERE CODE = 'WH_RAW'), m.ITEM_ID, m.SEED_QTY, SYSDATE);
    END IF;
    INSERT INTO INVENTORY_TRANSACTION
      (TXN_TYPE, ITEM_ID, WAREHOUSE_ID, QTY, BALANCE_AFTER, REF_NO, CREATED_BY)
    VALUES ('STOCK_IN', m.ITEM_ID, (SELECT ID FROM WAREHOUSE WHERE CODE = 'WH_RAW'),
            m.SEED_QTY, m.SEED_QTY, 'INIT_BALANCE', 'admin');
  END LOOP;

  -- The finished good must not exist in stock before the production order runs.
  DELETE FROM INVENTORY_TRANSACTION
   WHERE ITEM_ID = (SELECT ID FROM ITEM WHERE CODE = 'FG_RUNNER_PRO_42');
  DELETE FROM STOCK
   WHERE ITEM_ID = (SELECT ID FROM ITEM WHERE CODE = 'FG_RUNNER_PRO_42');
  COMMIT;

  DBMS_OUTPUT.PUT_LINE('>>> Step 0 SUCCESS: scenario reset (MAT_RUBBER_01 @ WH_RAW = 40, FG stock = 0)');
END;
/
